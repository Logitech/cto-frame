...
std::unique_ptr<frame::ProgramInterface> LoadProgramFromName(const std::string& name) {
    ...
    return LoadProgram(name, vertex_filepath, fragment_filepath, geometry_filepath);
}

std::unique_ptr<ProgramInterface> LoadProgramFromName(const std::string& program_name,
    ...
    return LoadProgram(program_name, vertex_filepath, fragment_filepath, geometry_filepath);
}

// TODO(anirul): Should be moved to the device.

std::unique_ptr<frame::ProgramInterface> LoadProgram(const std::string& program_name,
    ...
    if (geometry_filepath != "") try {
            ...
            return CreateProgram(program_name, vertex_ifs, fragment_ifs, geometry_ifs);
        } catch (std::runtime_error) {
        }
    return CreateProgram(program_name, vertex_ifs, fragment_ifs);
}
